...
static const char MyPluginName[] = "MaterialShader";
...
static void *MyCreateFunction(void)
{
  MaterialShader *pathtracing = new MaterialShader();
...
static void MyDeleteFunction(void *self)
{
  MaterialShader *pathtracing = (MaterialShader *) self;
  if (pathtracing == NULL) {
    return;
  }
  delete pathtracing;
...
void MaterialShader::evaluate(const TraceContext &cxt,
    const SurfaceInput &in, SurfaceOutput *out) const
{
...
Color MaterialShader::integrate_diffuse(const TraceContext &cxt,
      const SurfaceInput &in, SurfaceOutput *out) const
{
  Vector u, v, w;
  w = in.N;
...
Color MaterialShader::integrate_reflect(const TraceContext &cxt,
      const SurfaceInput &in, SurfaceOutput *out) const
{
  Vector R;
...
Color MaterialShader::integrate_refract(const TraceContext &cxt,
      const SurfaceInput &in, SurfaceOutput *out) const
{
  Vector T;
...
Color MaterialShader::direct_lighting(const TraceContext &cxt,
      const SurfaceInput &in, SurfaceOutput *out) const
{
#if 0
  /* THIS OLD VERSION */
  ...
  //TODO TEST
  /*
  static std::vector<LightSample> light_samples_bank[64];
  const int id = MtGetThreadID();
  std::vector<LightSample> &light_samples = light_samples_bank[id];
  const int nsamples = SlGetLightSampleCount(&in);
  light_samples.resize(nsamples);
  std::cout << light_samples.size() << "\n";
  ...
  for (int i = 0; i < nsamples; i++) {
    LightOutput Lout;
    SlIlluminance(&cxt, &light_samples[i], &in.P, &Nf, PI / 2., &in, &Lout);
